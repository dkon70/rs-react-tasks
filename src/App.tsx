      ...
      <>
        ...
        <main className={style.main}>
          <div className={`${style.wrapper} ${style.mainContainer}`}>
            {this.state.data.products && this.state.data.products.length > 0 ? this.state.data.products.map((el: Elem) => {
              return(
                <ProductCard key={el.title} title={el.title} thumbnail={el.thumbnail} description={el.description} price={el.price} />
              )
            }): this.state.firstLoad ? <h1></h1> : <h1>No Data</h1>}
          </div>
        </main>
      </>
    )
